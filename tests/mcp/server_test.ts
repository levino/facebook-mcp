...
import type { Tool, ToolContext } from "../../src/mcp/tools.ts";
  ...
  const tools: Tool[] = [
    {
      ...
      handler: (args, ctx) => {
        captured.ctx = ctx;
        return Promise.resolve(String(args.text ?? ""));
      },
    },
    {
      name: "boom",
...
Deno.test("tools/call returns text content and forwards the user context", async () => {
  const res = await makeServer().handle({
    jsonrpc: "2.0",
    id: 3,
    method: "tools/call",
    params: { name: "echo", arguments: { text: "hi" } },
  }, { userId: "alice" }) as JsonRpcSuccess;
  assertEquals(res.result.isError, false);
  assertEquals(res.result.content, [{ type: "text", text: "hi" }]);
  assertEquals(captured.ctx?.userId, "alice");
});
...
Deno.test("invalid JSON-RPC shape returns InvalidRequest", async () => {
  const res = await makeServer().handle({ foo: "bar" }, CTX) as JsonRpcError;
...
Deno.test("empty batch is InvalidRequest", async () => {
  const res = await makeServer().handle([], CTX) as JsonRpcError;
  assertEquals(res.error.code, -32600);
});
